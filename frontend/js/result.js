...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 4. Handle Empty State (with fallback)
  (async () => {
    if (!resultData) {
      // check URL params for an id
      const params = new URLSearchParams(window.location.search);
      const idParam = params.get("id") || params.get("prediction_id") || params.get("predictionId");
      if (idParam) {
        // attempt server fallback
        const serverItem = await tryLoadFromServerById(idParam);
        if (serverItem) {
          resultData = serverItem;
          // patient inputs won't be available; keep patientInputs null and the UI will show 'No patient data available'
          console.info("Loaded prediction from server fallback for id:", idParam);
        }
      }
